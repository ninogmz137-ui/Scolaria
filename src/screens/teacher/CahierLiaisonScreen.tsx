        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <HStack className="items-start gap-3 p-5 pb-0">
            ...
            <VStack className="flex-1">
              <Text className="text-lg font-extrabold mb-2" style={{ color: Colors.white }}>{selectedMot.titre}</Text>
          ...
          {/* Content card */}
          <Box className="m-5 p-4 rounded-2xl" style={{ backgroundColor: Colors.blueNightCard, borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)' }}>
            <Text className="text-xs mb-3" style={{ color: Colors.gray }}>
            ...
            <Text className="text-sm leading-[22px]" style={{ color: 'rgba(255,255,255,0.85)' }}>{selectedMot.contenu}</Text>
          </Box>
          ...
          {selectedMot.requires_signature && (
            <VStack className="px-5 gap-3">
              <HStack className="justify-between items-center">
                <Text className="text-base font-extrabold" style={{ color: Colors.white }}>Signatures</Text>
              ...
              {/* Progress bar */}
              <Box className="h-2 rounded-[4px]" style={{ backgroundColor: 'rgba(255,255,255,0.08)' }}>
                <Box
              ...
              <Text className="text-xs" style={{ color: Colors.gray }}>{sigPercent}% des parents ont signé</Text>

              {/* Signed list */}
              {signatures.length > 0 && (
                <Box className="p-3.5 rounded-[14px]" style={{ backgroundColor: Colors.blueNightCard, borderWidth: 1, borderColor: Colors.green + '20' }}>
                  ...
                  {signatures.slice(0, 10).map((sig) => (
                    <HStack key={sig.id} className="items-center gap-2 py-[5px]">
                      <Text className="text-[13px] flex-1" style={{ color: Colors.white }}>{sig.student_name}</Text>
                      <Text className="text-[11px]" style={{ color: Colors.gray }}>{formatDateTime(sig.signed_at)}</Text>
                    </HStack>
                  ))}
                  {signatures.length > 10 && (
                    <Text className="text-xs italic mt-2" style={{ color: Colors.gray }}>+ {signatures.length - 10} autres signatures</Text>
                  )}
                </Box>
              )}
              ...
              {unsignedStudents.length > 0 && (
                <Box className="p-3.5 rounded-[14px]" style={{ backgroundColor: Colors.blueNightCard, borderWidth: 1, borderColor: Colors.red + '30' }}>
                  ...
                  {unsignedStudents.map((s) => (
                    <HStack key={s.id} className="items-center gap-2 py-[5px]">
                      ...
                      <Text className="text-[13px] flex-1" style={{ color: Colors.white }}>{s.name}</Text>
                    </HStack>
                  ))}
                </Box>
              )}
  ...
  const createModal = (
    <Modal visible={showCreate} animationType="slide" transparent>
      <Box className="flex-1 justify-end" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
        <Box className="p-6 rounded-t-3xl" style={{ backgroundColor: Colors.blueNight, maxHeight: '90%', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)' }}>
          <ScrollView showsVerticalScrollIndicator={false}>
            {/* Modal header */}
            <HStack className="justify-between items-center mb-5">
              <Text className="text-lg font-extrabold" style={{ color: Colors.white }}>Nouveau mot</Text>
            ...
            {/* Type selector */}
            <Text className="text-[13px] font-bold mb-2 mt-4" style={{ color: Colors.gray }}>Type</Text>
                ...
                return (
                  <Pressable
                    ...
                    style={{
                      ...
                      backgroundColor: isActive ? tc.color + '20' : Colors.blueNightCard,
                      borderWidth: 1,
                      borderColor: isActive ? tc.color : 'rgba(255,255,255,0.06)',
                    }}
                  >
                    <Text className="text-lg">{tc.emoji}</Text>
                    <Text className="text-sm font-semibold flex-1" style={{ color: isActive ? tc.color : Colors.white }}>{tc.label}</Text>
                    {type !== 'info' && (
            ...
            {/* Titre */}
            <Text className="text-[13px] font-bold mb-2 mt-4" style={{ color: Colors.gray }}>Titre</Text>
            <TextInput
              style={{
                backgroundColor: Colors.blueNightCard, borderRadius: 14, padding: 14,
            ...
            {/* Contenu */}
            <Text className="text-[13px] font-bold mb-2 mt-4" style={{ color: Colors.gray }}>Message</Text>
            <TextInput
              style={{
                backgroundColor: Colors.blueNightCard, borderRadius: 14, padding: 14,
                color: Colors.white, fontSize: 14, minHeight: 120, textAlignVertical: 'top',
                borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)',
              }}
            ...
            {newType !== 'info' && (
              <>
                <Text className="text-[13px] font-bold mb-2 mt-4" style={{ color: Colors.gray }}>Date limite de signature (optionnel)</Text>
                <TextInput
                  style={{
                    backgroundColor: Colors.blueNightCard, borderRadius: 14, padding: 14,
                    color: Colors.white, fontSize: 14,
            ...
            {/* Classe info */}
            <HStack className="items-center gap-2 mt-4 p-3 rounded-xl" style={{ backgroundColor: Colors.blueNightLight }}>
              <Ionicons name="people" size={16} color={Colors.gray} />
              <Text className="flex-1 text-xs" style={{ color: Colors.gray }}>
                Sera envoyé à tous les parents de <Text style={{ color: Colors.white, fontWeight: '700' }}>CM2 B</Text> (25 élèves)
              </Text>
            </HStack>
    ...
    <Animated.View style={{ flex: 1, backgroundColor: Colors.blueNight, opacity: fadeAnim }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <HStack className="items-center gap-3.5 m-5 mb-3 p-4 rounded-2xl" style={{ backgroundColor: Colors.blueNightCard, borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)' }}>
          ...
          <VStack className="flex-1">
            <Text className="text-base font-extrabold" style={{ color: Colors.white }}>Cahier de Liaison</Text>
            <Text className="text-xs mt-0.5" style={{ color: Colors.gray }}>CM2 B · 25 élèves</Text>
          </VStack>
        </HStack>
        ...
        <HStack className="gap-2.5 mx-5 mb-4">
          <VStack className="flex-1 items-center p-3 rounded-[14px]" style={{ backgroundColor: Colors.blueNightCard, borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)' }}>
            ...
            <Text className="text-[10px] mt-0.5 text-center" style={{ color: Colors.gray }}>Mots envoyés</Text>
          </VStack>
          <VStack className="flex-1 items-center p-3 rounded-[14px]" style={{ backgroundColor: Colors.blueNightCard, borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)' }}>
            <Text className="text-xl font-black" style={{ color: Colors.green }}>{activeCount}</Text>
            <Text className="text-[10px] mt-0.5 text-center" style={{ color: Colors.gray }}>Actifs</Text>
          </VStack>
          <VStack className="flex-1 items-center p-3 rounded-[14px]" style={{ backgroundColor: Colors.blueNightCard, borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)' }}>
            <Text className="text-xl font-black" style={{ color: pendingSigCount > 0 ? Colors.red : Colors.green }}>
              {pendingSigCount}
            </Text>
            <Text className="text-[10px] mt-0.5 text-center" style={{ color: Colors.gray }}>Signatures{'\n'}en attente</Text>
          </VStack>
        </HStack>
        ...
        <HStack className="gap-2 mx-5 mb-4">
          {([['all', 'Tous'], ['envoyé', 'Actifs'], ['clos', 'Clos']] as const).map(([key, label]) => (
            <Pressable
              ...
              style={{
                backgroundColor: filter === key ? TEACHER_ORANGE + '20' : Colors.blueNightCard,
                borderWidth: 1,
                borderColor: filter === key ? TEACHER_ORANGE : 'rgba(255,255,255,0.06)',
              }}
            >
              <Text className="text-[13px] font-semibold" style={{ color: filter === key ? TEACHER_ORANGE : Colors.gray }}>{label}</Text>
            </Pressable>
          ))}
        </HStack>
        ...
        {filteredMots.length === 0 && (
          <VStack className="items-center p-10 gap-2">
            ...
            <Text className="text-sm" style={{ color: Colors.gray }}>Aucun mot dans cette catégorie</Text>
          </VStack>
        )}
        {filteredMots.map((mot) => {
          ...
          return (
            ...
            >
              ...
              {/* Title */}
              <Text className="text-[15px] font-bold mb-1" style={{ color: Colors.white }}>{mot.titre}</Text>
              <Text className="text-xs mb-2.5" style={{ color: Colors.gray }}>
              ...
              {mot.requires_signature && (
                <VStack className="gap-1">
                  <Box className="h-1.5 rounded-[3px]" style={{ backgroundColor: 'rgba(255,255,255,0.08)' }}>
                    <Box
